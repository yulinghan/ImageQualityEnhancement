...
Inoisy = np.float32(np.array(img))
Inoisy /=255.
print(Inoisy.shape)
restored = np.zeros_like(Inoisy)
restored_mask = np.zeros_like(Inoisy)
restored_mask += 1.0

width_range = int(Inoisy.shape[0] / 255)
height_range = int(Inoisy.shape[1] / 255)
print(width_range, height_range)
with torch.no_grad():
    for i in tqdm(range(width_range)):
        for k in range(height_range):
            noisy_patch = torch.from_numpy(Inoisy[i*255:(i+1)*255+1,k*255:(k+1)*255+1,:]).unsqueeze(0).permute(0,3,1,2).cuda()
            ...
            restored_patch *= 256.0
            mask_patch = np.zeros_like(restored_patch.numpy())
            mask_patch = 255.0
            restored[i*255:(i+1)*255+1,k*255:(k+1)*255+1,:] += restored_patch.numpy()
            restored_mask[i*255:(i+1)*255+1,k*255:(k+1)*255+1,:] += mask_patch

restored = restored / restored_mask
save_file = os.path.join(result_dir_img, '04d_02d.png')
cv2.imwrite(save_file, img_as_ubyte(restored))
