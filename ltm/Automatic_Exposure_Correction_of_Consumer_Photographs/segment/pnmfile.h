...
static image<uchar> *loadMat(Mat src) {
    int width = src.cols;
    int height = src.rows;

    /* read data */
    image<uchar> *im = new image<uchar>(width, height);
    memcpy((char *)imPtr(im, 0, 0), src.ptr(0), width * height * sizeof(uchar));
...
static Mat getMat(image<rgb> *im) {
    ...
    memcpy(dst.ptr(0), (char *)imPtr(im, 0, 0), width * height * sizeof(rgb));
