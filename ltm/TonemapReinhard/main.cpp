...
int main(int argc, char* argv[]) {
    ...
    float gamma = 1.0;
    float intensity = 0.0;
    float light_adapt = 0.0;
    float color_adapt = 0.0;

    Mat ldrReinhard;
	Ptr<TonemapReinhard> tonemapReinhard = createTonemapReinhard(1.0, 0, 0, 0);
	tonemapReinhard->process(img, ldrReinhard);
	imshow("ldr-Reinhard", ldrReinhard);

    MyTonemapReinhard *my_tonemap_reinhard_test = new MyTonemapReinhard();
    Mat out = my_tonemap_reinhard_test->Run(img, gamma, intensity, light_adapt, color_adapt);
