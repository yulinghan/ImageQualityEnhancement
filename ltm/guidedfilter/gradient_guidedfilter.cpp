...
Mat MyGradientGuidedfilterTest::Run(Mat I, Mat p, int r, float eps) {
    ...
    boxFilter(n_mat, n_mat, -1, Size(r, r), Point(-1, -1), true, BORDER_REFLECT);

    Mat mean_I, mean_p, mean_Ip, cov_Ip, mean_II, var_I;
    boxFilter(I, mean_I, -1, Size(r, r), Point(-1, -1), true, BORDER_REFLECT);
    mean_I = mean_I / n_mat;

    boxFilter(p, mean_p, -1, Size(r, r), Point(-1, -1), true, BORDER_REFLECT);
    mean_p = mean_p / n_mat;

    boxFilter(I.mul(I), mean_II, -1, Size(r, r), Point(-1, -1), true, BORDER_REFLECT);
    mean_II = mean_II / n_mat;

    boxFilter(I.mul(p), mean_Ip, -1, Size(r, r), Point(-1, -1), true, BORDER_REFLECT);
    ...
    boxFilter(n1_mat, n1_mat, -1, Size(r1, r1), Point(-1, -1), true, BORDER_REFLECT);

    Mat mean_I1, mean_II1, var_I1;
    boxFilter(I, mean_I1, -1, Size(r1, r1), Point(-1, -1), true, BORDER_REFLECT);
    mean_I1 = mean_I1 / n1_mat;

    boxFilter(I.mul(I), mean_II1, -1, Size(r1, r1), Point(-1, -1), true, BORDER_REFLECT);
    ...
    Mat mean_a, mean_b;
    boxFilter(a, mean_a, -1, Size(r, r), Point(-1, -1), true, BORDER_REFLECT);
    mean_a = mean_a / n_mat;

    boxFilter(b, mean_b, -1, Size(r, r), Point(-1, -1), true, BORDER_REFLECT);
    mean_b = mean_b / n_mat;
