...
MyFastTest::MyFastTest() {
}

MyFastTest::~MyFastTest() {
}

Mat MyFastTest::CornersShow(Mat src, vector<KeyPoint> corners) {
    Mat dst = src.clone();
...
void MyFastTest::MakeOffsets(int pixel[25], int rowStride, int patternSize)
{
...
int MyFastTest::CornerScore(const uchar* ptr, const int pixel[], int threshold)
{
...
void MyFastTest::Fast(Mat img, vector<KeyPoint>& keypoints, int threshold, bool nonmax_suppression) {
    int patternSize = 16;
...
vector<KeyPoint> MyFastTest::Run(Mat src) {
    vector<KeyPoint> keypoints;
    Fast(src, keypoints, 50, true);
